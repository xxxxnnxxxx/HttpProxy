...
ULONG BaseServer::stop() {
    ...
    for (int i = 0; i < m_ThreadCount; i++) {
        BOOL bRet = PostQueuedCompletionStatus(m_hCompletionPort, 
                                               0, 
                                               (ULONG_PTR)CONTROLCODE_SHUTDOWN, 
                                               0); //向所有的工作线程发送消息
        if (!bRet) {
            //错误处理
        }
...
ULONG BaseServer::http_server() {
    ...
    while (TRUE){      
        ...
        //传入的数据需要初始化  
        PACCEPTCONNECTION ac = new AcceptConnection(m_hCompletionPort, 
                                                    accept_socket, 
                                                    accept_addr, 
                                                    &m_HttpService_Params);
        ...
        DWORD RecvBytes = 0;
        DWORD Flags = 0;
        if (WSARecv(accept_socket, 
                    &ac->m_wsabuf[AcceptConnection::RECV], 
                    1, 
                    &RecvBytes, 
                    &Flags, 
                    (LPWSAOVERLAPPED)&ac->m_overlapped[AcceptConnection::RECV].overlapped, 
                    NULL) == SOCKET_ERROR) {
