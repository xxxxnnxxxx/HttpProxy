...
BaseHTTPRequestHandler::BaseHTTPRequestHandler(HTTPSERVICE_PARAMS *pHttpService_Params, HttpSession * pHttpSession)
    ...
    if (m_pHttpService_Params->bSSH) 
        m_pBaseSockeStream = new SSLSocketStream(&m_precv_buf, 
                                                 &m_len_recvbuf, 
                                                 &m_pHttpSession->m_pSendbuf, 
                                                 &m_pHttpSession->m_SizeofSendbuf);
    else
        m_pBaseSockeStream = new BaseSocketStream(&m_precv_buf, 
                                                  &m_len_recvbuf, 
                                                  &m_pHttpSession->m_pSendbuf, 
                                                  &m_pHttpSession->m_SizeofSendbuf);    //初始化基础的Socket处理器
}
...
void BaseHTTPRequestHandler::do_GET() {
    ...
    if (m_pHttpService_Params->bSSH)
        ret = httprequest.https_request(&http_items, 
                                        &httpcontent, 
                                        &response_httpheaders, 
                                        &response_httpcontent);
    else {
        if (_stricmp("SSLSocketStream", m_pBaseSockeStream->_classname(classname, 256)) == 0) {
            ...
            ret = httprequest.http_request(&http_items, 
                                           &httpcontent, 
                                           &response_httpheaders, 
                                           &response_httpcontent);
        }
...
size_t BaseHTTPRequestHandler::find_httpheader(const char* buf, size_t bufsize) {
    ...
    if (buf != NULL && bufsize>0) {
        if (ContentHandle::search_content(buf, 
                                          bufsize, 
                                          "^\\S{3,}\\s{1,}\\S{1,}\\s{1,}HTTP\\/[0-9]\\.[0-9]\\s{2}",
                                          NULL,
                                          NULL)) {
